...
public class Enviroment : MonoBehaviour
{
    ...
    private void gameOver(bool win)
    {
        ...
        if (win)
        {
            ...
            scoreWin.GetComponent<Text>().text = manageLanguagePhase.getScore() + score;
        ...
        if (countDownActionDays == 0 && day > 0)
        {
            ...
            switch (act)
            {
                ...
                case 1:
                    ...
                    {
                        toast.showToastNotCoins(manageLanguagePhase.getNoCoinsError());
                    }
                    break;
    ...
    private void applyAction(Action action, int valueGlobalEnviroment, Vector3 sizeQuadMessage, int cost)
    {
        if (score > cost)
        {
            ...
            nameEffect.GetComponent<Text>().text = manageLanguagePhase.getApplyingText() + getActionText(action) + "(" + countDownActionDays + " " + manageLanguagePhase.getDaysLeftText() + ")";
    ...
    private string getActionText(Action action)
    {
        ...
        {
            case Action.Atenuation:
                return manageLanguagePhase.getAttenuation();
            case Action.Bioestimulation:
                return manageLanguagePhase.getBioestimulation();
            case Action.Bioaumentation:
                return manageLanguagePhase.getBioaumentation();
            default:
                return "";
        }
    }
    ...
    {
        nameEffect.GetComponent<Text>().text = nameEffect.GetComponent<Text>().text.Replace("(" + d + " " + manageLanguagePhase.getDaysLeftText() + ")", "(" + (d - 1) + " " + manageLanguagePhase.getDaysLeftText() + ")");
    }
    ...
    private void updateStatusVariablesEnviroment()
    {
        ...
        moistureText.GetComponent<Text>().text = getMoistureText();
    ...
    private string getMoistureText()
    {
        switch (variables.moisture)
        {
            case Moisture.low:
                return manageLanguagePhase.getMoistureLow();
            case Moisture.medium:
                return manageLanguagePhase.getMoistureMedium();
            case Moisture.high:
                return manageLanguagePhase.getMoistureHigh();
            default:
                return variables.moisture.ToString().ToUpper();
